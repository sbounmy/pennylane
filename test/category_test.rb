...
class CategoryTest < Test::Unit::TestCase
  ...
  class ListTest < CategoryTest
    test "#list" do
      categories = vcr { Pennylane::Category.list }
    ...
    test 'without api key raises error'  do
      ...
      assert_raises Pennylane::AuthenticationError do
        vcr { Pennylane::Category.list }
      end
    end

    test 'can overwrite config api key' do
      Pennylane.api_key = nil
      categories = vcr { Pennylane::Category.list({}, api_key: 'x0dFJzrOYxeJgzEddjT1agJl1K9T4C-kpjg7qpuKyEs') }
      assert categories.count > 13
    end

    test 'accepts filter' do
      list = vcr { Pennylane::Category.list(filter: [{field: 'label', operator: 'eq', value: 'Virements internes'}]) }
  ...
  class RetrieveTest < CategoryTest
    ...
    test 'raise error when not found' do
      assert_raises Pennylane::NotFoundError do
        vcr { Pennylane::Category.retrieve('not_found') }
      end
    end

    test "return object when found" do
      cat = vcr { Pennylane::Category.retrieve('7a38e5f3-e523-40c6-a80b-79eddf943072') }
  ...
  class CreateTest < CategoryTest
    test 'create category' do
      Timecop.freeze '2024-05-26' do
        # Dont expire because we dont want to re-record creation because label has to be unique.
        # Change timecop datetime to record new cassette
        vcr expire: false do |cassette|
          label = "New Category (#{Time.now.to_i})"
          category = Pennylane::Category.create group_source_id: 'bd4c493f-d4f4-4033-8505-8d95d8649fda', label: label
          expected = Pennylane::Category.retrieve(category.source_id)
